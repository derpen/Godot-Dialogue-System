...
func _physics_process(_delta: float) -> void:
	if player_state == PLAYER_STATE.WALKING:
		...
		# velocity = (Vector3.RIGHT * input_velocity_x * move_speed) + (Vector3.FORWARD * input_velocity_z * move_speed)
		velocity = -transform.basis.z * move_speed * input_velocity_z ## Forward is -z
		velocity += transform.basis.x * move_speed * input_velocity_x
